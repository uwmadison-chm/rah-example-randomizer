...
from rah_example_randomizer import handler
...
def write_secrets(tmp_path):
    """Write a secrets TOML file and return its path.
    ...
    def write(contents: dict[str, str] | None = None):
        if contents is None:
            contents = {
                ...
                "redcap_api_token": "SECRET-TOKEN-DO-NOT-LOG",
            }
...
@pytest.fixture
def make_config(write_secrets):
    def build(secrets_path=None, **overrides):
        if secrets_path is None:
            secrets_path = write_secrets()
        config = {
            "redcap_secrets_file": str(secrets_path),
...
def make_context(tmp_path):
    ...
    def build(config, slug="randomize"):
...
@pytest.fixture
def make_message():
    def build(record_id="R-1001", body_text=None, message_id="<msg-1@example.edu>"):
        if body_text is None:
            body_text = f'record_id = "{record_id}"\n'
        return Message(
...
@pytest.fixture
def read_stored_row():
    """Read (value, completed_at) for a record straight out of the sqlite file."""

    def read(state_dir, record_id):
        db_path = state_dir / "randomizations.sqlite3"
